...
from utils import compute_bpr_loss, compute_entropy_loss, compute_metric, construct_link_prediction_data_by_node, construct_link_prediction_data_by_node_fairwalk
...
from evaluate import eval_unbiasedness_pokec, eval_unbiasedness_movielens, eval_unbiasedness_twitter, eval_unbiasedness_polblog, eval_unbiasedness_nba, eval_unbiasedness_cora
...
def arg_parse():
    ...
    parser.add_argument('--dataset', type=str, default='pokec-z', choices=['movielens', 'pokec-z', 'pokec-n','twitter','pol-blog','dblp','nba','cora']) 
    ...
    parser.add_argument('--beta', type=float, default=0.2, help='acceptance rate')
...
def learn_embeddings(args):
    ...
    #link_pred_data_dict = construct_link_prediction_data_by_node_fairwalk(data_name=data_name)
    ...
    if debias_method in ['uge-r', 'uge-c']:
        ...
        nobias_attr_comb_groups_map = {tuple(group[1].iloc[0][non_sens_attr_ls]):list(group[1].index) for group in nobias_comb_groups}
    ...
    for e in pbar:
        ...
        if args.loss == 'entropy':  #change the loss function appropriately to add the regularization loss funtion
        ...
        if debias_method in ['uge-r', 'uge-c']:
            regu_loss = 0
            scr_groups = random.sample(list(attr_comb_groups_map.keys()), 4)  
            dst_groups = random.sample(list(attr_comb_groups_map.keys()), 4)
    ...
    return path, test_pos_g, test_neg_g
...
def eval_embeddings(args, out_embed_path):
    ...
    if args.dataset == 'movielens':
        ...
        eval_unbiasedness_twitter(args.dataset, out_embed_path)
    elif args.dataset == 'pol-blog':
        eval_unbiasedness_polblog(args.dataset, out_embed_path)
    elif args.dataset == 'dblp':
        eval_unbiasedness_dblp(args.dataset, out_embed_path)
    elif args.dataset == 'nba':
        eval_unbiasedness_nba(args.dataset, out_embed_path)
    elif args.dataset == 'cora':
        eval_unbiasedness_cora(args.dataset, out_embed_path)
    else:  #pokec
...
if __name__ == '__main__':
    ...
    args = arg_parse()
    #out_embed_path, test_pos_g, test_neg_g = learn_embeddings(args)
    out_embed_path = "./embeddings/nba_gat_entropy_0.01_800_uge-r_age_0.1_fairwalk.bin"
    #test_pos_edges = list(test_pos_g.edges().cpu().numpy()) 
    ...
    '''
    pos_g_src = test_pos_g.edges()[0].cpu().numpy()
    pos_g_dst = test_pos_g.edges()[1].cpu().numpy()
    m1 = pos_g_src.shape
    
    neg_g_src = test_neg_g.edges()[0].cpu().numpy()
    neg_g_dst = test_neg_g.edges()[1].cpu().numpy()
    m2 = neg_g_src.shape
    
    accum_acc = 0
    for i in range(pos_g_src.shape):
        if final_mtx[pos_g_src[i]][pos_g_dst[i]]==1:
            accum_acc = accum_acc + 1 
            
    for i in range(neg_g_src.shape):
        if final_mtx[neg_g_src[i]][neg_g_dst[i]]==1:
            accum_acc = accum_acc + 1 
    accuracy = accum_acc/(m1 + m2)
