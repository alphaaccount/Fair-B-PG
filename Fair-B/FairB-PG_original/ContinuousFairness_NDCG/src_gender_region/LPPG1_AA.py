...
#     Lp_prob += 2*X[n+1]+10*X[n+2]+9*X[n+3]+3*X[n+4]
    #alpha=0.8
    #beta_avg = 0.10
    Lp_prob+= p.lpSum([(X[j])*sigmoid(data.iloc[j,6])*adamic_adar[j] for j in range(n)]) 
...
#     report_index(index,data1,e):  
    ...
    beta_all = [0.9,0.8,0.6,0.4,0.2]
    for beta in range(20):
      print("--------------This is for beta=",beta,"---------------------------")
      gr = [ [] for _ in range(m) ]
      ...
      cost = cp.kl_div(x,b)
      for i in range(m):
        constraints += [cp.multiply(x,data1[i]) <= (random_beta[i])*sizes[i]]
        #coonstraints += [cp.multiply(x,data[i]) >= 0.5*sizes[i]]
      constraints += [x>=0, x<=1]
      #print(constraints)
      #print(cost)
...
def QPG_AA(data,adj_mtx, beta_avg):     #here we are taking two sensitive attrib # make the data here  
    ...
    G = nx.from_numpy_matrix(adj_mtx.toarray())
...
#     report_index(index,data1,e):  
    ...
    for i in range(n):
      for j in range(2):
        for k in range(2):
          if data.iloc[i,1]==j and data.iloc[i,4]==k and sizes[2*j+k]>0:
            adamic_weight[i] = a_adar_gr[2*j+k]/sizes[2*j+k]
      for j in range(6):
        for k in range(6):
          if data.iloc[i,1]==j and data.iloc[i,4]==k and sizes[6*j+k+4]>0:
            adamic_weight[i] = a_adar_gr[6*j+k+4]/sizes[6*j+k+4]
    ...
    for beta in beta_all:
      ...
      for indicator in indicator_all:
        ...
        data_np = data1
        constraints = []
        cost1 = cp.sum_squares(cp.multiply(adamic_weight,(x - b)))
...
data = pd.read_csv('allPokec_gender_region.csv', sep=',')
b = sigmoid(data.iloc[:,6].to_numpy())
epsilon = 0.01
#for i in epsilon:
