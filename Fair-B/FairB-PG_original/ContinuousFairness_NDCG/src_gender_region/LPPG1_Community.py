...
def LPG(data,adj_mtx, epsilon):     #here we are taking two sensitive attrib # make the data here  
    ...
    community_list = list(community.label_propagation_communities(G))
    com_num = len(community_list)
    com_score = np.ones(n)
    print("The number of communities are", com_num)

    community_num = np.zeros(M)
    for i in range(M):
      for j in range(com_num):
        if (i in community_list[j]):
           community_num[i] = j

    for i in range(n):
      u = int(data.iloc[i,0])
      v = int(data.iloc[i,3])
      if community_num[u]==community_num[v]:
        com_score[i] = 2

    print("Communitites found")
...
#     Lp_prob += 2*X[n+1]+10*X[n+2]+9*X[n+3]+3*X[n+4]
    #alpha=0.8
    #beta_avg = 0.10
    Lp_prob+= p.lpSum([(X[j])*sigmoid(data.iloc[j,6])*com_score[j] for j in range(n)]) 
...
#     report_index(index,data1,e):  
    ...
    for beta in beta_all:
      ...
      for indicator in indicator_all:
        ...
        data_np = data1
        constraints = []
        cost1 = cp.sum_squares(cp.multiply(com_score,(x - b)))
