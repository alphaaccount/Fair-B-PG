...
def find_dp_ndcg(data, adj_mtx):
    ...
    pair_scores = data.iloc[:,6].to_numpy()
    num_sample_pairs = n
    epsilon = 0.01
    ...
    for i in range(len(pair_scores)):
      if pair_scores_sorted[i] >= topbeta:
        if pair_scores_sorted[i] > topbeta_eps:
          pair_scores_algo[i]=1
        elif pair_scores_sorted[i] < topbeta_eps:
    ...
    for i in range(n):
        fair_scores.append(pair_scores_algo[i])
...
def find_dp_ndcg_all(data, adj_mtx):
    ...
    embedding_np = pk.load(open('../data/pokec-z_gat_entropy_0.01_800_uge-r_AGE_0.1.bin', 'rb'))
