...
def LPG(data,adj_mtx, beta_avg):     #here we are taking two sensitive attrib # make the data here  
...
#     Lp_prob += 2*X[n+1]+10*X[n+2]+9*X[n+3]+3*X[n+4]
    #alpha=0.8
    #beta_avg = 0.10
    Lp_prob+= p.lpSum([(X[j])*sigmoid(data.iloc[j,6]) for j in range(n)]) 
    ...
    for i in range(m):
      #  if i<m:
            ...
            Lp_prob += p.lpSum([(X[j])*(data1[i][j]) for j in range(n)]) <= (Y[i]+0.278)*sizes[i]
